...
public class SSActionMineBlock extends SSAction {
     ...
     */
    public SSActionMineBlock(Block[] blockIDs, int maxNum, Item[] miningTools) {
        BIDs = blockIDs;
        maxCollect = maxNum;
        toolIDs = miningTools;
    }
    ...
    public void doAction(SSEntityScriptedSurvivor ess) {
        ...
        else if ((ess.actionTimer >= 101 && ess.actionTimer <= 120 && ess.getEntityToAttack() == null)
    ...
    private boolean isTargetBlock(SSEntityScriptedSurvivor ess, int i, int j, int k) {
        for (Block BID : BIDs)
            if (ess.worldObj.getBlock(i, j, k) == BID)
                return true;
        return false;
    }
    ...
    private void selectPreferredTool(SSEntityScriptedSurvivor ess) {
        ess.actionTimer = 20;
        ess.setPathToEntity(null);
        for (Item toolID : toolIDs) {
            if (ess.pack.setCurrentItem(toolID)) return;
        }
            ...
            if (ess.nums[0] > 1.0F) { //block destruction complete
                ...
                if (flag1) {
                    ...
                    for (int j1 = 0; j1 < i1; j1++) {
                        Item k1 = block == Blocks.snow ? Items.snowball : block.getItemDropped(0, ess.worldObj.rand, 0);
                        if (k1 != null) {
                            float f1 = 0.7F;
                            ...
                            ess.worldObj.spawnEntityInWorld(entityitem);
                            ess.setTarget(entityitem);
                        }
                    }
                }
    ...
    public SSAction createAction(String s) {
        try {
            ...
            Block a[] = new Block[codes1.length];
            int num = Integer.valueOf(codes[1]).intValue();
            Item b[] = new Item[codes2.length];
            //block ids
            for (int i = 0; i < codes1.length; i++) {
                a[i] = Block.getBlockFromName(codes1[i]);
            }
            //tool ids
            for (int i = 0; i < codes2.length; i++) {
                b[i] = Item.getItemById(Integer.parseInt(codes2[i]));
            }
    ...
    private Item[] toolIDs; //item ids of tools to use for mining the block
}

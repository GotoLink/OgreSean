...
public class SSActionSayMessage extends SSAction{
	...
	public void exitAction(SSEntityScriptedSurvivor ess, boolean actionCanceled){
		...
		//say message
		String s = mod_ScriptedSurvivors.scripts.get(ess.type).name.concat(": ").concat(messages[ess.rand.nextInt(messages.length)]);
	...
	public SSAction createAction(String s){
		try{
			...
			int a = Integer.valueOf(codes[0]).intValue();

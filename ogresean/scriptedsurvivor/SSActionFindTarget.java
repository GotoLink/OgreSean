...
public class SSActionFindTarget extends SSAction{
	 ...
	 */
	public SSActionFindTarget(String[] targets, int maxTicks, int[] weapons){
	...
	public void doAction(SSEntityScriptedSurvivor ess){
		if(ess.getEntityToAttack() == null && (ess.actionTimer == -1 || ess.path == null || ess.rand.nextFloat() < 0.001F)) setNewWayPoint(ess);
		
		if((ess.getEntityToAttack() == null || !ess.canEntityBeSeen(ess.getEntityToAttack()) || ess.getDistanceSqToEntity(ess.getEntityToAttack()) > 256D)&& ess.actionTimer % 10 == 0) checkForTargets(ess);
		else if(ess.getEntityToAttack() != null && weaponIDs != null && weaponIDs[0] != -1) attackTarget(ess);
		
		if(ess.swingProgress > 0) ess.swingProgress = ess.swingProgress >= 0.875F ? 0.0F : ess.swingProgress + 0.125F;
		if(ess.getEntityToAttack() != null && !ess.getEntityToAttack().isEntityAlive()) ess.setEntityToAttack(null);
		ess.actionTimer++;
	}
	
	
	public boolean isActionComplete(SSEntityScriptedSurvivor ess){
		if(ess.getEntityToAttack() != null && weaponIDs != null && weaponIDs[0] != -1)
			return false;
	...
	private void attackTarget(SSEntityScriptedSurvivor ess){
		if(!ess.canEntityBeSeen(ess.getEntityToAttack()) || ess.getEntityToAttack() instanceof EntityItem) return;
		if(ess.getHeldItem() != null && ess.getHeldItem().getItem() == Items.bow && ess.getDistanceSqToEntity(ess.getEntityToAttack()) < 200D){
            ...
            if(ess.attackTime == 0 && ess.pack.consumeInventoryItem(Items.arrow))
            {
                ...
                ess.worldObj.playSoundAtEntity(ess, "random.bow", 1.0F, 1.0F / (ess.rand.nextFloat() * 0.4F + 0.8F));
                ess.worldObj.entityJoinedWorld(entityarrow);
                entityarrow.setArrowHeading(d, (d2 + (double)f1), d1, 1.4F, 1.5F);
                ess.attackTime = 10 + ess.rand.nextInt(20); //added random attacktime
            }
			...
			ess.attackTime = 24 - ess.rand.nextInt(14); 
			ess.prevSwingProgress = ess.swingProgress;
			ess.swingProgress = 0.0001F;
			ItemStack itemstack = ess.pack.getCurrentItem();
			int damage = itemstack == null || itemstack.itemID < 256 ? 2 : itemstack.getItem().getDamageVsEntity(ess.getEntityToAttack());
	...
	private void setWeapon(SSEntityScriptedSurvivor ess){
		for(int i = 0; i < weaponIDs.length; i++){
			if(ess.pack.setCurrentItem(weaponIDs[i])) return;
		}
	...
	private boolean isEntityTarget(Entity e){
		...
		for(int i = 0; i < targetedEntities.length; i++){
			if((targetedEntities[i].equalsIgnoreCase("player") && e instanceof EntityPlayer)
					|| targetedEntities[i].equalsIgnoreCase(e.getEntityString())
					|| targetedEntities[i].equalsIgnoreCase("all")) return true;
		}
		return false;
	}
	...
	public SSAction createAction(String s){
		try{
			...
			int weapons[] = new int[weps.length];
			...
			for(int i = 0; i < weps.length; i++){
				weapons[i] = Integer.valueOf(weps[i].trim()).intValue();
			}

...
public class EntityTalkingPig extends EntityPig {
    ...
    public void updateEntityActionState() {
        if (!worldObj.isRemote) {
            if (riddenByEntity instanceof EntityLivingBase) {
                ...
                try {
                    if (Boolean.class.cast(jump.get(entityliving))) {
                        if (onGround) {
                            onGround = false;
                            jump();
    ...
    protected Entity findPlayerToAttack() {
        if ((commands & 2) != 2)
            return null;
        @SuppressWarnings("unchecked") List<Entity> list = worldObj.getEntitiesWithinAABBExcludingEntity(this, boundingBox.expand(14D, 14D, 14D));
        double dist = 9999;
        Entity target = null;
    ...
    protected void handlePals() {
        ...
        if (pals.size() < followers) {
            double d = 16D;
            @SuppressWarnings("unchecked") List<Entity> list = worldObj.getEntitiesWithinAABBExcludingEntity(this, boundingBox.expand(d, d, d));
    ...
    private void attractEnemies() {
        @SuppressWarnings("unchecked") List<Entity> list = worldObj.getEntitiesWithinAABBExcludingEntity(this, boundingBox.expand(10D, 10D, 10D));
        for (Entity entity : list) {
            if (!(entity instanceof EntityMob)) {
                continue;
            }
